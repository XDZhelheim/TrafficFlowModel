...
lock=mp.Lock()
np_mat=np.zeros((len(df_roads), len(df_roads)), dtype=np.int32)
...
def get_track(file_path):
    ...
    df_taxi, coords=resample_coords(df_taxi)
    ...
    df_taxi["track"]=track
    df_taxi.to_csv("../data/taxi/tracks/{}".format(file_path.split("/")[-1]), index=False)
    
    print("../data/taxi/tracks/{}".format(file_path.split("/")[-1]))
...
if __name__ == "__main__":
    ...
    for file in os.listdir("../data/taxi/processed/")[:N]:
        file_path="../data/taxi/processed/"+file
        res=pool.apply_async(get_track, (file_path,))

    pool.close()
    pool.join()

    np.save("../data/trans_matirx.npy", trans_matrix)
